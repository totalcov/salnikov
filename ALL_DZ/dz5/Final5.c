...
struct Btree
{
    struct Btree* pLeft; // left and right children
    struct Btree* pRight;
    int val; // value
};
...
/* НОВЫЙ ВЫВОД - как в предыдущих примерах */
void printTreeVisual(Btree* root, int space, int isRight)
{
    if (root == NULL) return;
...
/* Старый вывод для отладки */
void printTreeOld(Btree* root, int level)
{
    int i;
    if (!root) return;

    printTreeOld(root->pRight, level + 1);
    ...
    printTreeOld(root->pLeft, level + 1);
}
...
int main(void)
{
    ...
    printf("1. ВСТАВКА И БАЛАНСИРОВКА ДЕРЕВА:\n");
    printf("========================================\n");

    // генерируем и выводим 15 случайных чисел
    int values[15];
    printf("Генерируем 15 случайных чисел: ");
    for (int i = 0; i < 15; i++) {
        values[i] = genRand();
        printf("%d ", values[i]);
    }
    ...
    int test_values[] = {41, 23, 18, 26, 40, 69, 66, 86, 3};
    ...
    */
    ...
    printf("\n2. ИТОГОВОЕ СБАЛАНСИРОВАННОЕ ДЕРЕВО:\n");
    printf("────────────────────────────────────────\n");
    printTreeVisual(root, 0, 0);
    printf("────────────────────────────────────────\n");
    ...
    if (root != NULL) {
        ...
        printf("\nБаланс дерева:\n");
        printf("  Левых узлов: %d\n", leftCount);
        printf("  Правых узлов: %d\n", rightCount);
    ...
    printf("\n3. ОТРАЖЕННОЕ ДЕРЕВО (зеркальное):\n");
    printf("────────────────────────────────────────\n");
    printTreeVisual(root, 0, 0);
    printf("────────────────────────────────────────\n");
    ...
    if (root != NULL) {
        int leftCount = countNodes(root->pLeft);
        int rightCount = countNodes(root->pRight);
        printf("\nБаланс отраженного дерева:\n");
        printf("  Разница между поддеревьями: %d\n", abs(leftCount - rightCount));
    }
